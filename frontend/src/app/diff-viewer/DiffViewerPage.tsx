...
export function DiffViewerPage() {
  ...
  const suggestCommitDisabledReason = resolveCommitSuggestionDisabledReason({
    comparisonMode,
    stagedFileCount: visibleFiles.filter((file) => file.hasStagedChanges).length,
    hiddenStagedFileCount,
    isCheckingAgents: aiSettingsState.isCheckingAgents,
    hasProvider: Boolean(commitFeatureState?.provider),
    providerValid: commitFeatureState?.providerValid ?? false,
    providerReason: commitProviderStatus?.reason,
  })
  ...
  return (
    ...
    >
      <section className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <DiffViewerStatusBanners
          headerError={headerError}
          isMergeInProgress={mergeState.inProgress}
          isConflictMode={isConflictMode}
          mergeModeSummary={mergeModeSummary}
          showMergeResolvedState={showMergeResolvedState}
        ...
        >
          {isPullRequestSurface ? (
            ...
            <WorkingTreeSurface
              isBranchCompareLoading={isBranchCompareLoading}
              selectedBaseRef={selectedBaseRef}
              selectedFile={selectedFile}
              currentDiff={currentDisplayedDiff}
              comparisonMode={comparisonMode}
