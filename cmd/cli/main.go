...
func resolveRepoLayout() (repoLayout, error) {
	...
	if err == nil {
		if layout, layoutErr := newRepoLayout(currentDir); layoutErr == nil {
			return layout, nil
		}
	}
...
func newRepoLayout(repoRoot string) (repoLayout, error) {
	...
	if _, err := os.Stat(filepath.Join(absoluteRepoRoot, "go.mod")); err != nil {
		return repoLayout{}, fmt.Errorf("invalid repo root %q: missing go.mod", absoluteRepoRoot)
	}
...
func runInstall(layout repoLayout) error {
	if err := runCommand("backend", layout.repoRoot, "go", "mod", "download"); err != nil {
		return err
	}
...
func runBuild(layout repoLayout) error {
	...
	buildOutputDir := filepath.Join(layout.repoRoot, "cmd", "bin")
